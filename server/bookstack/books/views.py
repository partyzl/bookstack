...
class UserRead(APIView):
    ...
    def get(self, request, username, format=None):
        ...
        read = list(
            Book.objects.raw(
                "SELECT * FROM books_book WHERE user_id_id IS %s AND date_finished IS NOT NULL",
                [user.id],
            )
        )
...
class UserCurrent(APIView):
    ...
    def get(self, request, username, format=None):
        ...
        read = list(
            Book.objects.raw(
                "SELECT * FROM books_book WHERE user_id_id IS %s AND date_started IS NOT NULL AND date_finished IS NULL",
                [user.id],
            )
        )
        serializer = BookSerializer(read, many=True)
